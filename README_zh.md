...
一个为**真实长期使用**而设计的 Pomodoro + To-Do 专注工具。
...
- 🍅 自动切换 Pomodoro / ☕ Short Break / 🛏️ Long Break
- ⏰ 基于时间戳计算，避免浏览器后台节流机制
- ⏯️ 计时过程中可手动 暂停 / 重新开始 / 完成
- 📝 To-Do 管理（进行中 / 已完成）
- 📊 **历史数据统计与完成记录展示**
  - 🍅 番茄钟累计完成次数
  - 🎯 专注总时长
...
- **🎯 任务驱动专注:**  
  Pomodoro 需要**绑定**一个 To-Do，没有任务就无法开始计时。
- **⏰ 计时实现说明**
  本项目使用 Date.now() + 目标时间戳计算剩余时间，而非依赖 setInterval 递减，从根本上避免浏览器[后台节流机制](https://developer.chrome.com/blog/timer-throttling-in-chrome-88?hl=zh-cn)导致的计时不准问题。
- **🔒 无需账号、无需联网:**  
...
本项目部署在 GitHub Pages 上，由于 Pages 为静态托管，不支持 SPA 的 history 路由回退，因此在非根路径（如 /todos）下刷新页面会出现 **_404_**。

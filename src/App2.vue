...
{{ setting }}
	...
	<div id="body">
		<div id="Pomodoro">
			...
			<div id="time_control">
				<button type="button" :style="{ display: isCountdown ? 'none' : 'flex' }" @click="Start">
		...
		<footer>
			<time :datetime="time.date">{{ time.date }}&nbsp</time>
			<time :datetime="time">
				<span>{{ time.hour }}</span>
...
import { currentTodo } from "./store/todo";
...
function updateTime() {
	...
	time.date = info.date;
	time.hour = info.hour;
	time.minute = info.minute;
	time.second = info.second;
